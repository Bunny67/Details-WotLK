...
function lib:CacheGameData ()
	...
	for class_id = 1, GetNumClasses and GetNumClasses() or 0 do
		...
		for idx = 1, GetNumSpecializationsForClassID (class_id) do
			...
			if specIDs[class] and specIDs[class][idx] then
				gspec_id = specIDs[class][idx]
			end
			...
			gspec.role = global_spec_id_roles_detailed[gspec_id]
		end
...
function lib:ProcessQueues()
	...
	if not next(mainq) and next(staleq) then
		--[===[@debug@
		debug ("Main queue empty, swapping main and stale queues")
		--@end-debug@]===]
	...
	if (self.state.last_inspect + INSPECT_TIMEOUT) < GetTime () then
		...
		if guid then
			...
			elseif count > MAX_ATTEMPTS then
				--[===[@debug@
				debug ("Excessive retries, moving to stale queue")
				--@end-debug@]===]
	...
	for guid, count in pairs(mainq) do
		...
		elseif not CanInspect(unit) or not UnitIsConnected(unit) then
			--[===[@debug@
			debug ("Cannot inspect "..unit..", aka "..(UnitName(unit) or "nil")..", moving to stale queue")
			--@end-debug@]===]
			mainq[guid], staleq[guid] = nil, 1
		else
			--[===[@debug@
...
local function GetSpecialization(isInspect)
...
function lib:BuildInfo(unit)
	...
	local specIndex = is_inspect and GetSpecialization(unit) or GetSpecialization()
	if specIDs[class] and specIDs[class][specIndex] then
		info.global_spec_id = specIDs[class][specIndex]
	end
...
function lib:INSPECT_TALENT_READY(guid)
	local unit = self:GuidToUnit(guid)
...
function lib:UpdateCommScope ()
	...
	local scope = (instanceType == "pvp" and "BATTLEGROUND") or (IsInRaid () and "RAID") or (IsInGroup () and "PARTY")
...
function lib:ACTIVE_TALENT_GROUP_CHANGED(unit)
		...
		if not UnitIsUnit(unit, "player") then
			if UnitIsVisible(unit) then
				if info.not_visible then
					...
					--[===[@debug@
					debug(unit..", aka "..(UnitName(unit) or "nil")..", is now visible")
					--@end-debug@]===]
					if not self.state.mainq[guid] then
						self.state.staleq[guid] = 1
						self.frame:Show()
...
local spellName = GetSpellInfo(63644)
function lib:UNIT_SPELLCAST_SUCCEEDED(unit, name)
