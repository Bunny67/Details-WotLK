...
function DF:GetAuraByName (unit, spellName, isDebuff)
	...
	for i = 1, 40 do
		local name, rank, texture, count, debuffType, duration, expirationTime, caster, canStealOrPurge, shouldConsolidate, spellId = UnitAura (unit, i, isDebuff)
		if (not name) then
			return
		end

		if (name == spellName) then
			return name, rank, texture, count, debuffType, duration, expirationTime, caster, canStealOrPurge, shouldConsolidate, spellId
		end
	end
end
...
-------- automatic
	...
	f_auto:SetScript ("OnShow", function()
		for i = 1, BUFF_MAX_DISPLAY do
			local name, _, texture, count, debuffType, duration, expirationTime, caster, _, nameplateShowPersonal, spellId = UnitAura ("player", i, "HELPFUL")
			if (name) then
				ALL_BUFFS [spellId] = true
			end
			local name, _, texture, count, debuffType, duration, expirationTime, caster, _, nameplateShowPersonal, spellId = UnitAura ("player", i, "HARMFUL")
			if (name) then
